...
```{r}
Ecological_chinese <- read.csv("Ecological_correlates_of_extinction_risk_in_Chinese_amphibians.csv") %>%  
                        select(-Species) #  already corrected iucn2020_binomial according to https://www.amphibiachina.org/
```
...
```{r}
setdiff(Ecological_chinese$iucn2020_binomial, Chinese_species_distribution_grid$species) # species not have distribution
...
fviz_screeplot(mfa,addlabels = TRUE,ncp=8, ggtheme = theme_bw())
...
amphibian_FE <- ggplot()+
    ...
    scale_fill_distiller(palette = "Spectral",na.value="#5b5b5b",limits=c(0.4, 1))+ # fill 
    geom_sf(data = phyloregions_line,aes(geometry = geometry,col=attr,size = attr))+
    scale_color_manual(values = c('red','#F75404', 'grey60'),guide = "none")+ # delete legends of divider
    scale_size_manual(values = c(0.8,0.6,0.5),guide = "none")+
...
```{r}
Distinctness <- as.data.frame(funrar$Di) %>%
    rownames_to_column(var = 'grid_id')
Dinstinctness_long <- gather(Distinctness, key = 'species', value = 'distinctness', -grid_id) %>%
    filter(!is.na(distinctness))
```
...
top_FRic_species <- filter(Dinstinctness_long, grid_id %in% top_FRic_grid)  %>%
                        ...
                        filter(grid_area < thresold_area & result.category %in% c('VU', 'EN', 'CR', 'EX', 'EW')) %>%
                        slice(1:3)
...
```{r}
Ui_distribution <- filter(Chinese_species_distribution_grid,
       species %in% unique(row.names(chinese_traits_imputed))) %>% 
    dplyr::select(grid_id,species) %>%
    as.data.frame() %>%
...
```{r}
filter(Ui_distribution, Ui_mean > 3)
```
...
fig_FD <- ggpubr::ggarrange(amphibian_FD, amphibian_FV, amphibian_FE,amphibian_Ui, labels = c("A", "B", "C", "D"),
...
```{r}
save(amphibian_Ui,amphibian_FD, amphibian_FV, amphibian_FE, 
