...
Chinese_species_distribution_grid <- read.csv("../chinese_terrestral/species_distribution_grid_filtered.csv",
                                        ...
                                        filter(!(familyName %in% toupper(c('Procellariidae','Alcidae','Fregatidae','Gaviidae',
                                                                           'Phaethontidae','Diomedeidae', 'Stercorariidae',
                                                                           'Sulidae','Pelecanidae','Phalacrocoracidae','Laridae',
                                                                           'Hydrobatidae')))) %>% # remove marine species
                                        select("class","species","grid_id")

n_distinct(Chinese_species_distribution_grid$species)
```

# traits data integration
...
avonet <- read.csv('ELEData/TraitData/AVONET1_BirdLife.csv') %>%
                select(Species1, Trophic.Niche, Hand_Wing.Index) %>% 
...
Chinesebirdsdata <- mutate(Chinesebirdsdata,
...
addition <- filter(elton_birds, Scientific %in% name) %>%
                left_join(Chinesebirdsdata, by = c('Scientific'='elton_bird_name'), keep = TRUE) %>%
                select(-Scientific)
total_addition <- filter(total, !(elton_bird_name %in% addition$elton_bird_name)) %>%
                    rbind(addition) %>% 
                    mutate(iucn_2021_binomial = gsub('_', ' ', iucn_2021_binomial))
```

## filter Chinese birds

```{r}
chinese_traits_origin <- filter(total_addition,iucn_2021_binomial %in% Chinese_species_distribution_grid$species,
                                !(family %in% c('Procellariidae','Alcidae','Fregatidae','Gaviidae',
                                                'Phaethontidae','Diomedeidae','Stercorariidae','Sulidae',
                                                'Pelecanidae','Phalacrocoracidae','Laridae','Hydrobatidae'))) # filter terrestrial birds
```
...
## traits clean and selection

```{r}
# summary(chinese_traits_origin)
chinese_traits <- mutate(chinese_traits_origin,
                    # recode,transform categorical variables to factors
...
```{r}
traits_cat <- data.frame(trait_name = colnames(chinese_traits_missmfa$completeObs), 
...
ggsave('PCA/quality.fspaces.plot.png', height = 7, width = 28, units = 'in', dpi=300)
ggsave('PCA/quality.fspaces.plot.svg', height = 7, width = 28)
```
...
mfa <- FactoMineR::MFA(chinese_traits_missmfa$completeObs,
                       ...
                       type = c(rep('s', 8), rep('n', 4)),
                       name.group = c('body_size', 'Maximum_longevity', 'Clutch_size', 'Diets', 'ForStrat', 'Hand_Wing.Index',
...
fviz_screeplot(mfa, addlabels = TRUE, ncp=15, ggtheme = theme_bw())
...
fviz_mfa_ind(mfa, col.ind = "cos2", 
             ...
             repel = TRUE,legend = c(0.9,0.85), ggtheme = theme_bw())
...
```{r}
bird_pca_var_plot <- fviz_pca_var(pca, col.var = "contrib",gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
            geom = c("arrow", "text"), labelsize=6, repel = TRUE, ggtheme = theme_bw()) # Correlation between quantitative variables and dimensions
bird_pca_var_plot <- ggpubr::annotate_figure(bird_pca_var_plot, 
                                               fig.lab = "A",fig.lab.size = 18)
bird_pca_var_plot
...
```{r}
fast_slow_cwm <- FD::functcomp(ind_pca,Chinese_species_distribution_grid_selected) %>%
...
fast_slow_cwm_map_abnormal <- filter(fast_slow_cwm_map,LGM_sat_velocity > 5, LGM_sat_velocity < 6.5, Dim.1 > 0.75)

bird_fast_slow_cwm_map <- ggplot()+
    geom_sf(data = china_map_std,aes(geometry = geometry),colour='black',fill='grey90',size=0.2)+
    ...
    labs(fill = "Dim.1")+
    custom_theme
bird_fast_slow_cwm_map_abnormal <- ggplot()+
    geom_sf(data = china_map_std,aes(geometry = geometry),colour='black',fill='grey90',size=0.2)+
...
```{r}
lm_eqn <- function(m){
  eq <- substitute(italic(r)^2~"="~r2~", p < 0.001",
...
bird_cwm_fast_slow <- ggpubr::ggarrange(cwm_p1,cwm_p2, ncol = 1,nrow = 2, common.legend = TRUE)
bird_cwm_fast_slow
...
fig_FD <- ggpubr::ggarrange(bird_FD, bird_FV, bird_FE, bird_Ui, labels = c("A", "B", "C", "D"),
...
```{r}
save(bird_Ui, bird_FD, bird_FV, bird_FE,
     file = "bird_FD.Rdata")
```

```{r}
save(bird_pca_var_plot, bird_cwm_fast_slow,bird_fast_slow_cwm_map_abnormal, bird_fast_slow_cwm_map,
     file = "bird_fast_slow_cwm.Rdata")
```
