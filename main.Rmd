...
lizard_phylog <- read.csv("./phylogenetic_result/reptile_phylogenetic_diversity.csv") %>% 
                    select(grid_id, reptile_ntaxa.x, reptile_NRI, reptile_NTI)
...
```{r}
lm_fit <- lm(mammal_NRI~prec + tavg + LGM_sat_velocity + LGM_dO_velocity + X50km_range + pop_10000BC_value,data = total_table)

#create vector of VIF values
vif_values <- car::vif(lm_fit)

svglite::svglite(filename = "Fig/vif.svg", width = 7, height = 5)
#create horizontal bar chart to display each VIF value
barplot(vif_values, main = "VIF Values", horiz = TRUE, col = "steelblue",las=2,
       names.arg = c('modern prec.','modern temp.','LGM temp. change','LGM prec. change','elevation range','historical Anthrop.'))
...
model_result$class[model_result$class == 'lizard'] <- 'reptile'
...
model_result_main <- filter(model_result,diversity_index %in% c('NRI','NTI','FRic','FDiv','FEve')) %>% 
    mutate(p_value = case_when(
        ...
        class == 'reptile' & diversity_index %in% c('FRic', 'FDiv', 'FEve') ~ pt(abs(t_value), 3650, lower.tail = FALSE),
        class == 'reptile' & diversity_index %in% c('NRI', 'NTI') ~ pt(abs(t_value), 3848, lower.tail = FALSE),
...
model_result_main$class<- factor(model_result_main$class, 
                                 levels = c("amphibian","reptile","bird","mammal"))
...
p_main
ggsave('Fig/model_result_main.png', plot = p_main, height = 6, width = 11, units = 'in', dpi=300)
ggsave('Fig/model_result_main.svg', plot = p_main, height = 6, width = 11)
```
