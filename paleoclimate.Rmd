...
china_map <- st_read('StandardMap_China/Base_SingleChinaRegion_GS20204615_2.shp') %>%
    st_transform(crs = st_crs(grid)) %>% 
    mutate(line = tidyr::replace_na(line, 0)) %>% filter(line != 2)
...
```{r}
paleovalue <- read.csv('environment_variables.csv')
```
...
# ggsave('Fig/LGM_sat_velocity.png',width = 7,height = 7,units = 'in',dpi = 300)
ggsave('Fig/LGM_sat_velocity.svg',width = 7,height = 7)
```
